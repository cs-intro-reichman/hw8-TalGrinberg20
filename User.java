 ...
 *  a list of user names that s/he follows, and the list's size. */
 public class User {
    ...
    public User(String name, boolean gettingStarted) {
        this(name); //calls the cunstructor with the string input 
        follows[0] = "Foo";
        follows[1] = "Bar";
        follows[2] = "Baz";
        fCount = 3;
...
public boolean removeFollowee(String name) {
    ...
    public int countMutual(User other) {
        int result = 0;
        for (int thisIndex = 0; thisIndex < this.fCount; thisIndex++) {
            for (int otherIndex = 0; otherIndex < other.fCount; otherIndex++) {
                if (this.follows[thisIndex].equals(other.follows[otherIndex])) {
                    result++;
                }
            }
        }
